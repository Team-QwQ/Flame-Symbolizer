...
- 默认符号格式改为 `short`（函数名 + 文件基名 + 行号），方便定位源码；如需避免因文件名差异拆分同一符号，可显式选择 `none`；`full` 则保留路径。
...
- `--location-format`：控制输出是否携带文件与行号，`short`（默认）输出函数名和文件名（无路径）与行号，`none` 表示仅输出函数名（可减少因文件名差异拆分），`full` 输出函数名和带路径的文件名与行号。
