...
- **输出**：与输入行数一致；若地址解析成功，frame 被函数签名替代；解析失败则维持原 `0x...` 片段。
- 默认符号格式包含函数名和短文件名+行号（文件名不带路径前缀）；可通过可选参数关闭文件/行号，或改为携带路径前缀。
...
6. **可见性与告警**
   ...
   - 提供可选调试开关，开启后在 `stderr` 打印已解析的段表、命中的符号目录及解析决策（例如命中文件、调整后的地址偏移）。
   - 调试输出不应影响正常 stdout 结果，且可控关闭以便流水线集成。
   - 符号缺失类告警：找不到模块二进制，或该模块的二进制无符号表导致 addr2line 产出 `??`，默认也要输出；对每个模块/段仅告警一次并缓存为不可解析，避免刷屏。
...
- `--location-format`：控制输出是否携带文件与行号，`none` 表示仅输出函数名，`short` 输出函数名和文件名（无路径）与行号，`full` 输出函数名和带路径的文件名与行号；默认 `short`（即默认输出 file:line，且文件名不带路径）。
...
- 找不到模块二进制、或该模块的二进制无符号表导致 addr2line 产出 `??` 时，应输出警告（默认模式也输出；“二进制/模块二进制”均指 maps 中的可执行映像），并在该模块/地址段上只告警一次后缓存为不可解析以避免刷屏。
