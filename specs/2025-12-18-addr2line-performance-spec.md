...
- **I/O 模式与缓冲**：
  ...
  - 支持省略 --output 触发原地覆盖：为避免数据丢失，先写同目录临时文件并 fsync 后再 mv 覆盖；不支持直接用 stdout。
- **子进程调用开销**：
...
- **缓存与内存占用**：
  - 地址缓存、模块缓存当前未实现上限/禁用开关，仍需评估并记录缓存尺寸与风险；如需限流留待后续扩展。
- **健壮性**：
  - 当前未提供可配置的超时/限流开关；若 `addr2line`/`readelf` 失速、挂起，仅依赖现有失败回退与告警继续流水线。可配置超时留待后续扩展。
...
## 验收标准
- 基于现有夹具测试仍然通过（当前唯一必跑验证）。
- 大规模合成输入（≥40k 行、多模块、多地址）的性能验证与耗时记录建议后续补充，但当前实现未覆盖此验证，视为已知缺口。
- 在 `--debug` 模式下，应能看到行进度或定位停滞区间；无 debug 模式时性能不应显著劣化。
